...
После завершения развёртывания была выполнена итоговая проверка инфраструктуры относительно требований дипломного задания.

| Требование | Реализация | Статус |
|---|---|:---:|
| Использование Terraform и Ansible | Инфраструктура создаётся Terraform, настройка серверов выполняется Ansible | ✅ |
| Inventory использует FQDN `.ru-central1.internal` | Все VM в Ansible Inventory указаны через внутренние FQDN | ✅ |
| Два web-сервера | `web-01` и `web-02` | ✅ |
| Web-серверы в разных зонах | `ru-central1-d` и `ru-central1-a` | ✅ |
| nginx на обеих VM | Установлен и используется как reverse proxy | ✅ |
| Идентичное содержимое web-серверов | На обеих VM развёрнуто одинаковое FastAPI-приложение | ✅ |
| Web-VM без публичных IP | Обе VM находятся в приватных подсетях | ✅ |
| SSH-доступ через Bastion | Используется Bastion Host и ProxyCommand | ✅ |
| Application Load Balancer | Развёрнут и доступен из интернета | ✅ |
| Target Group | Содержит `web-01` и `web-02` | ✅ |
| Backend Group | Настроена для обоих web-серверов | ✅ |
| HTTP Router | Маршрутизация `/` настроена | ✅ |
| Listener TCP/80 | Настроен | ✅ |
| Проверка приложения через публичный ALB | Выполнена, HTTP 200 | ✅ |
| Health Check backend | HTTP/80 `/health` | ⚠️ |
| Проверка отказоустойчивости web-уровня | При остановке nginx на `web-01` ALB продолжал работу через `web-02` | ✅ |
| Zabbix Server | Развёрнут на отдельной VM | ✅ |
| Zabbix Agent 2 | Установлен на всех VM | ✅ |
| Мониторинг CPU | Настроен | ✅ |
| Мониторинг RAM | Настроен | ✅ |
| Мониторинг файловых систем | Настроен | ✅ |
| Мониторинг сети | Настроен | ✅ |
| HTTP-мониторинг сайта | Выполняется через ALB | ✅ |
| Trigger недоступности сайта | Настроен и практически проверен | ✅ |
| Elasticsearch | Развёрнут на отдельной приватной VM | ✅ |
| Filebeat | Установлен на `web-01` и `web-02` | ✅ |
| Сбор `nginx access.log` | Выполняется | ✅ |
| Сбор `nginx error.log` | Выполняется | ✅ |
| Kibana | Развёрнута на отдельной VM | ✅ |
| Kibana подключена к Elasticsearch | Подключение работает | ✅ |
| Просмотр логов в Kibana | Проверен через Discover | ✅ |
| Один VPC | Используется `diploma-network` | ✅ |
| Web и Elasticsearch в приватной сети | Публичные IP отсутствуют | ✅ |
| Zabbix и Kibana доступны извне | Используются публичные IP и отдельные Security Groups | ✅ |
| Bastion имеет публичный IP | Используется статический адрес `158.160.202.52` | ✅ |
| SSH Bastion ограничен | TCP/22 разрешён только с административного `/32` | ✅ |
| Security Groups разделены по назначению | Bastion, web, Zabbix, Elasticsearch, Kibana и PostgreSQL используют отдельные правила | ✅ |
| NAT Gateway | Настроен для выхода приватных VM в интернет | ✅ |
| Автоматические snapshots | Настроены для всех 8 рабочих VM | ✅ |
| Периодичность snapshots | Ежедневно в `01:00 UTC` | ✅ |
| Retention snapshots | 7 суток | ✅ |
| Токен Yandex Cloud отсутствует в Git | Авторизация выполнялась через environment variables | ✅ |
| Постоянные VM перед сдачей | На текущем этапе используется `preemptible = true` | ❌ |
...
#### Что необходимо сделать непосредственно перед сдачей

Остаётся одно обязательное изменение — отключить режим прерываемых виртуальных машин.

На этапе разработки использовалось:

```hcl
preemptible = true
```

Перед сдачей VM должны быть переведены в постоянно работающий режим:

```hcl
preemptible = false
```

Изменение необходимо сначала проверить через:

```bash
terraform plan
```

и только после проверки плана применять к инфраструктуре.
